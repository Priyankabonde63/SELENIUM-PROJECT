...
public static void main(String[] args) throws InterruptedException {
	...
	for (WebElement d : dash) {
	  if (d.getText().equals("COINS")) {
		...
		List<WebElement> menu =driver.findElements(By.xpath("((//span[contains(text(),'10 gram')]/ancestor::div)[4]/descendant::ul)[2]/li"));
		for (WebElement subMenu : menu) 
		{
			a.moveToElement(subMenu).perform();		
			Thread.sleep(2000);
			if (subMenu.getText().equals("50 gram")) {
				driver.findElement(By.xpath("(//span[contains(text(),'50 gram')])[1]")).click();
				break;
			}
		}
			break;
	...
	if (title.equals("50 gram 24 KT Gold Coin | BlueStone.com")) {
		...
		System.out.println("FAIL:50 Gram coin displayed");
	}
}
}
